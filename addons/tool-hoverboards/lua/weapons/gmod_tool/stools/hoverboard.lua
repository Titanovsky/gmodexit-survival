...
if ( SERVER ) then
	...
	function MakeHoverboard( pl, model, ang, pos, mcontrol, shake, height, viewdist, trailsize, trail, boost, recharge, attributes )
		...
		attributes.speed = math.Clamp( attributes.speed, 1, 32 )
		attributes.jump = math.Clamp( attributes.jump, 0, 32 )
		attributes.turn = math.Clamp( attributes.turn, 1, 64 )
		attributes.flip = math.Clamp( attributes.flip, 1, 32 )
		attributes.twist = math.Clamp( attributes.twist, 1, 32 )
