Ambi.General.Hash = Ambi.General.Hash or {}

-- -------------------------------------------------------------------------------------
-- Source: https://github.com/SuperiorServers/dash/blob/master/lua/dash/libraries/hash.lua
...
function Ambi.General.Hash.MD5( sStr )
    local msgLen = #sStr
    ...
    sStr = sStr .. char(128) .. rep(char(0),padLen-1) .. lei2str(8*msgLen) .. lei2str(0)
    local t = CONSTS
    local a,b,c,d = t[65],t[66],t[67],t[68]

    for i=1,#sStr,64 do
    local X = cut_le_str(sub(sStr,i,i+63),4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4)
...
local HH = {}
function Ambi.General.Hash.SHA256( sMsg )
    sMsg = preproc( sMsg, #sMsg )
    local H = initH256(HH)
    for i = 1, #sMsg, 64 do
        digestblock( sMsg, i, H )
    end
...
function Ambi.General.Hash.PseudoRandom( nSeed )
