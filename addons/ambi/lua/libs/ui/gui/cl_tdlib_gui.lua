...
-- Doc: https://threebow.gitbooks.io/tdlib
...
-- --------------------------------------------------------------------------------------------------------------------------------------------
local function HoverFunc( vguiPanel ) 
    return vguiPanel:IsHovered() 
end

local function drawCircle( nX, nY, nRadius )
    local circle = {}

    for i = 1, 360 do
        circle[i] = {}
        circle[i].x = nX + math.cos( math.rad( i * 360 ) / 360 ) * nRadius
        circle[i].y = nY + math.sin( math.rad( i * 360 ) / 360 ) * nRadius
    end
...
classes.Hook = classes.On -- by Titanovsky
...
classes.FadeHover = function(pnl, col, speed, rad)
    ...
    pnl:SetupTransition("FadeHover", speed, HoverFunc)
...
classes.BarHover = function(pnl, col, height, speed)
    ...
    pnl:SetupTransition("BarHover", speed, HoverFunc)
...
classes.FillHover = function(pnl, col, dir, speed, mat)
    ...
    pnl:SetupTransition("FillHover", speed, HoverFunc)
    pnl:On("PaintOver", function(s, w, h)
        surface.SetDrawColor(col)
...
classes.CircleHover = function(pnl, col, speed, trad)
    ...
    pnl:SetupTransition("CircleHover", speed, HoverFunc)
    pnl:On("Think", function(s)
        if(s:IsHovered()) then
...
classes.AvatarMask = function(pnl, mask)
    ...
    pnl.SetPlayer = function(s, ply, size) s.Avatar:SetPlayer(ply, size) return pnl end
    pnl.SetSteamID = function(s, id, size) s.Avatar:SetSteamID(id, size) return pnl end
end
...
classes.CircleFadeHover = function(pnl, col, speed)
    ...
    pnl:SetupTransition("CircleFadeHover", speed, HoverFunc)
    pnl:On("Paint", function(s, w, h)
        draw.NoTexture()
...
classes.CircleExpandHover = function(pnl, col, speed)
    ...
    pnl:SetupTransition("CircleExpandHover", speed, HoverFunc)
    pnl:On("Paint", function(s, w, h)
...
classes.End = function() end -- by Titanovsky
...
function Ambi.UI.GUI.DrawTDLib( sClass, vguiParent, sName )
    local panel = vgui.Create( sClass, vguiParent, sName )
